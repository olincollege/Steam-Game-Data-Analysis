...
def get_game_data():
    """
    Gets info about a game from its link and adds it to a file.
    """
    pass


def get_html_data(game_link):
    """
    Retrieves all the data of a game's reviews from its path and adds it
    to a file.

    Args:
        game_link: a string representing the path to the file containing
        the game's data in html format

    Returns:
        A list with each element containing the reviews data from that game.
    """
    soup = BeautifulSoup(open(game_link), "html.parser")
